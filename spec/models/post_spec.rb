...
describe Post do
  ...
  describe 'self#search_by_column' do
    ...
    it 'returns posts with matching titles' do
      ...
      expect(Post.search_by_column('Preview')).to eq([audi_q7, mercedes_ml])
    end
    ...
    it 'returns the posts of the correct page' do
      ...
      expect(Post.search_by_column(nil, '2')).to eq([ubuntu, windows, os_x])
    end
    ...
    it 'returns first page posts if no page number entered' do
      ...
      expect(Post.search_by_column()).to eq(page_one_posts.reverse)
    end
  end
